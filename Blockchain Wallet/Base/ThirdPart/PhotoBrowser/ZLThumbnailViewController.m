...
- (void)panAction:(UIPanGestureRecognizer *)pan
{
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (_beginSelect) {
            ...
            [self canAddImgModel:m canLoad:^(bool canLoad) {
...
//    __block NSString *canContinue = nil;
...
//
//            [PJToastView showInView:[UIApplication sharedApplication].keyWindow text:@"所选图片不能大于5MB" duration:2 autoHide:YES];
//            canLoad = NO;
//            canContinue = @"YES";
...
- (void)canAddImgModel:(ZLPhotoModel *)model canLoad:(void (^)(bool canLoad))canLoad{
    ZLImageNavigationController *nav = (ZLImageNavigationController *)self.navigationController;
    ZLPhotoConfiguration *configuration =nav.configuration;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.selectedBlock = ^(BOOL selected) {
        ...
        if (!selected) {
            //选中
//            signal = dispatch_semaphore_create(0);
//            [self canAddImgModel:model canLoad:^(bool canLoad) {
//                if (canLoad) {
